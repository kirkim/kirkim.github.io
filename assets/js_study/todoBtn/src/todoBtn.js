...
export class TodoBtn {
	...
	makePage(num, title) {
		...
		const newTodo = new Todo(tempForm, num, title);
		const newPage = {
			fm: tempForm,
			nb: num,
		}
	...
	userInput() {
		const input = prompt('제목을 입력하세요', 'title');
		return input;
	}
	...
	loadData() {
		...
		if (savedData !== null) {
			...
			parseData.forEach((obj) => this.makePage(obj.nb, obj.title));
		}
	}
}
