...
<h3>3. 코드예시(특이케이스)</h3>
<h5>(1)&#60; sscanf함수로 문자형정수를 int형정수로 변환하기 &#62;</h5>
...
<br /><br />
<h5>(2)&#60; 서식자와 맞지않는 데이터가 들어온 경우(특징3) &#62;</h5>
...
<br /><br />
<h5>(3)&#60; scanf함수의 버퍼오버플로우 해결법(특징2) &#62;</h5>
...
* sscanf함수에서 서식자오류로 무한루프에 빠질 위험에 빠지더라도 fgets함수를 다시 만나기 때문에 무한루프에 빠질 위험이 없어지게 됩니다.
...
while (TRUE)
{
	if (fgets(result, BUFFER, stdin) == NULL)
	{
		clearerr(stdin);
		break;
	}
	if (sscanf(result, "%d %d", &num, &num2) == 1)
		printf("%d %d\n", num, num2);
}
```