...
* `color`를 **수직, 수평**벽에 따라 색을 다르게해서 칠해주도록 했습니다.
* 하지만 이번에는 <rd>동, 서, 남, 북</rd>에 따라 다른 벽의 색이 칠해주도록 `color`대신 새로운 함수를 구현할 예정입니다.
...
* **3D랜더링**에 필요한 **변수**<b style="font-size:85%">(길이, 모니터까지 길이, 어안렌즈보정 길이 등등)</b>들을 <rd>초기화 함수</rd>에서 초기화할 수 있도록 구현하였습니다.
...
int     set_wall_color(t_god *god, t_3d *v, int i)
{
    ...
    if (direction == T_WE || direction == T_EA)
        col = (int)god->ray.wall_hitY % (int)god->parse.tex[direction].width;
    else
        col = (int)god->ray.wall_hitX % (int)god->parse.tex[direction].width;
	/* 줄(row) 좌표 보정 */
