...
- <b><rd>비교해야될 부분은</rd></b> 위의 비동기함수들을 최종적으로 사용한 <b class="blue">다음의 코드들</b>입니다.
...
- <b class="green">똑같게 동작</b>하는 코드입니다.
...
- 위와 같이 `a`변수들에 미리 선언하여 사용했습니다.
- 주의할 점은 **동기적**으로 사용하기 이전에 `await`로 작업을 기다려주는 작업이 필요합니다.
