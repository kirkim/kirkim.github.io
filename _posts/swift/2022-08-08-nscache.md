...
⛔️ Swift에서 NSCache에 대한 개인적인 생각을 적은 글입니다. 그렇기 때문에 가볍게 읽어주셨으면 좋겠습니다.<br />
...
**디스크캐시**는 하드디스크에 접근하는시간을 개선하기위해 RAM에 저장, **메모리캐시**는 RAM에 접근하는 시간을 개선하기위해 CPU에 내장된 캐시메모리에 저장한다고 합니다. 앱도 웹과 용어만 다를뿐 비슷한 개념이라고 생각합니다.<br />
캐시는 기술적인 한계때문에 처리속도와 용량은 반비례합니다. 그렇기 때문에 목적에 맞게 캐시를 사용하는 것이 중요합니다.<br />
이번 포스트에서 다룰 <b class="purple">NSCache</b>는 **메모리캐시**입니다.
가장 큰 특징은 앱이 종료(혹은 백그라운드상태)되면 모든 캐시데이터가 삭제된다는 것입니다.

<h1 class="ksubject">2️⃣ NSCache 사용하기</h1>
<h2 class="ksubsubject">NSCache를 적용하기전 스크롤뷰 모습</h2>
iOS의 ScrollView의 셀들은 재사용되는 특성이 있습니다. 만약 캐시를 사용하지 않는다으면 다음과 같이 셀이 재사용될때마다 이미지를 네트워크로부터 새롭게 요청하게 됩니다. 특히 네트워크로 받아오는 데이터의 경우 하드디스크에서 가져오는 시간보다 훨씬 많은시간이 걸리고 비용에 직접적으로 영향을 주기 때문에 캐시의 사용이 거의 필수적입니다.
...
NSCache를 이용한 캐시매니저를 싱글턴클래스로 만들었습니다. 기능은 필요한 부분인 Create, Read 구현했습니다.
...
위에서 NSCache매니저를 구현하면서 느낀점은 NSCache와 Dictionary가 작동하는 방식이 비슷해 보입니다. 다음은 Read, Create부분을 NSCache와 Dictionary로 구현하여 비교한 모습입니다.
...
연결리스트가 사용되기는 하지만 큰틀로 봤을때 데이터가 딕셔너리형태로 저장되어 있습니다.
하지만 NSCache에 관한 문서를 살펴보면 다음과 같이 일반적인 collections와 다르다고 합니다.
...
**NSCache**는 키와 값으로 <b class="brown">AnyObject</b>만 받고 있습니다. 이를 내부적으로 복사하지않고 참조형태로 저장하고 있습니다.
...
이것을 확인하기 위해 **Dictionary**를 대조군으로 놓고 실험을 해봤습니다.<br />
...
Dictionary는 다음의 짤을 보면 알듯이 <rd>쓰레드 안전하지 않습니다.</rd> 첫번째크래쉬는 **update**과정에서 두번째 크래쉬는 **키값을 불러오는 과정**에서 크래쉬가 일어난 것 같습니다.
...
내부구현을 보면 countLimit과 totalCostLimit값이 정확하지 않다고 주석으로 설명하고 있습니다.
...
<b class="blue">totalCostLimit</b>도 역시 <b class="blue">countLimit</b>과 마찬가지로 정확한 값으로 관리되지는 않습니다.
